...
import { fetchContractForSigning, signContractServer } from "@/lib/signing.functions";
...
function SignPage() {
  ...
  useEffect(() => {
    fetchContractForSigning({ data: { token } })
      .then((result) => {
        if (result.kind === "invalid") {
          setState({ kind: "invalid", reason: result.reason });
        } else if (result.kind === "signed") {
          setState({ kind: "signed", contract: result.contract! });
        } else {
          setState({ kind: "ready", contract: result.contract!, pdfUrl: result.pdfUrl });
        }
      })
      .catch(() => {
        setState({ kind: "invalid", reason: "Link inválido ou expirado. Solicite um novo ao remetente." });
      });
  }, [token]);
  ...
  async function sign() {
    ...
    setSubmitting(true);
    try {
      const result = await signContractServer({ data: { token, name: fullName.trim() } });
      if (!result.success || !result.contract) {
        toast.error("Não foi possível assinar. O link pode ter expirado.");
      } else {
        setState({ kind: "signed", contract: result.contract });
      }
    } catch {
